			...
			if (options.scene == '1038' && options.referrerInfo.appId == 'wxef277996acc166c3' && this.initIn) {
				...
				if (!extraData) {
					...
					if (extraData.code == 'success') {
						let url =
							`/pages/goods/order_pay_status/index?order_id=${this.orderId}&msg=${this.jumpData.msg}&type=3&totalPrice=${this.payPriceShow}`
		...
		methods: {
			...
			goPay(number, paytype) {
				...
				orderPay({
					...
					let goPage = '/pages/goods/order_pay_status/index?order_id=' + this.orderId + '&msg=' + res
						...
						case 'WECHAT_PAY':
							...
							if (jsConfig.code_url) {
							   console.log(encodeURIComponent(jsConfig.code_url))
								...
								// });
							uni.navigateTo({
								url: `/pages/annex/web_view/index?url=${encodeURIComponent(jsConfig.code_url)}`
							});
							} else {
							...
							// #endif
							// #ifdef APP-PLUS
							//如果当前环境是app 那么jsConfig.code_url 就是支付链接 但是必须要在微信中打开 怎么实现
							uni.hideLoading();
							console.log(jsConfig.code_url)
							if (jsConfig.code_url) {
